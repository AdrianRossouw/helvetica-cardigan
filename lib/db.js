...
db.addWord = function(json, callback) {
...
db.updateWord = function(json, callback) {
    if (!json.id) {
        return callback("Make sure everything has an id!");
    }

    client.hmset('word:'+json.id, json, callback);
};

// players

/*
db.getPlayers = function(fridgeId) {
};

db.getPlayerIds = function(fridgeId) {
};
*/

db.getPlayer = function(id) {
    client.hgetall('player:'+id, callback);
};

db.addPlayer = function(json, callback) {
    if (!json.id || !json.fridgeid) {
        return callback("Make sure everything has an id!");
    }
    client.multi()
        .sadd('fridgeid:'+json.fridgeid+'playerids', json.id)
        .hmset('player:'+json.id, json)
        .exec(callback);
};

db.updatePlayer = function(json, callback) {
    if (!json.id) {
        return callback("Make sure everything has an id!");
    }
    client.hmset('player:'+json.id, json, callback);
};
